...
class E2ESupModel(nn.Module):
    def __init__(self, n_classes):
        super(E2ESupModel, self).__init__()
...
def main():
    ...
    parser.add_argument('--lr_cls', type=float, default=1e-1, help='learning rate for classifier')
    parser.add_argument('--opti', type=str, default='Adam', choices=['SGD', 'Adam', 'LARS'], help='optimizer to be used')
    parser.add_argument('--batchsize', type=int, default=64, help='batchsize for E2ESup evaluation')
    ...
    prefix = f'E2ESup-lrE[{args.lr_enc}]_LrC[{args.lr_cls}]_ep[{args.epochs}]_opt[{args.opti}]_bs[{args.batchsize}]'
    ...
    outfile.write(f"Testset report | {args.model} E2ESup eval | {args.epochs} epochs \n\n")
    ...
    print('Model and logs saved -- E2ESup evaluation complete!!')
